...
def load_patient(subj, tmpl):
    ...
    # print('RRRRRRROOOOOOOOOOIIIIIIIIIIIIIII')

    # print(len(ROIs))   ## 392 for cc400 

    functional = np.nan_to_num(df[ROIs].to_numpy().T).tolist()


    # print( len(functional[0]))    # 196 for cc400 & cc200

    # print(functional[0]) 
    # print( len(functional[1]))
    functional = preprocessing.scale(functional, axis=1)

    # print(len(functional[0]))
    # print(functional[0])


    functional = compute_connectivity(functional)

    # print(functional.shape) # 76636 for cc400 
...
if __name__ == "__main__":
    ...
    hdf5 = hdf5_handler(bytes("./data/abide_trial.hdf5",encoding="utf-8"), 'a')
